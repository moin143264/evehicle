...
cron.schedule('* * * * *', async () => {
  ...
  try {
    ...
    bookings.forEach(async (booking) => {
      ...
      const startTime = moment(`${booking.selectedDate.toISOString().split('T')[0]}T${booking.startTime}:00`).utc();
      const endTime = booking.endTime ? moment(`${booking.selectedDate.toISOString().split('T')[0]}T${booking.endTime}:00`).utc() : null;
      ...
      else if (endTime && endTime.diff(now, 'minutes') <= 0 && booking.status !== 'expired') {
